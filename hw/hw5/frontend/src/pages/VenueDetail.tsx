    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <p className="font-semibold">{new Date(venue.date).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">{t('venue.location')}</p>
          <p className="font-semibold">{venue.location.venue}</p>
          <p className="text-sm">{venue.location.city}, {venue.location.country}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">{t('venue.price')}</p>
          <p className="font-bold text-xl text-blue-600">${venue.ticketPrice}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">{t('venue.available')}</p>
          <p className="font-semibold">{venue.availableTickets} {t('venue.tickets')}</p>
        </div>
      </div>

      <div className="bg-gray-100 p-4 rounded-lg mb-6">
        <label className="block mb-2 font-semibold">{t('venue.quantity')}</label>
